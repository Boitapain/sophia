...
import { SophiaFSM, SophiaFsmError } from "../src/index.js";
describe("Sophia FSM Gatekeeper Tests", () => {
    ...
    test("Interdiction de stager ou review depuis l'état IDLE", () => {
        const fsm = new SophiaFSM();
        assert.throws(() => fsm.stageChanges({ target_file: "test.ts", diff_or_code: "// some diff" }), (err) => {
            return err instanceof SophiaFsmError && err.message.includes("PLANNED");
        });
        assert.throws(() => fsm.reviewAndLock({ review_notes: "valid", is_approved: true }), (err) => {
